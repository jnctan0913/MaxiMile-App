...
- **Auto-capture delivered via iOS Shortcuts + Android Notifications**: SGFinDex not available to fintechs; implemented native OS integration workaround — reduces logging from 20 sec to 0-3 sec
...
Top 20 SG miles cards at MVP; 29 cards (85% market coverage) by v1.5. Rules updated within 48 hours of detected changes. Version-controlled with change history. Community-sourced verification layer. This database is the core operational asset that creates the data moat.

**5. Auto-Capture via iOS Shortcuts + Android Notifications (F26 + F27 — Shipped)**
iOS: Apple Pay transaction trigger launches MaxiMile with pre-filled amount, merchant, and card in 2-3 seconds. Android: NotificationListenerService parses bank SMS notifications and auto-logs transactions in 0 seconds (passive). Reduces manual logging from 20 sec to 0-3 sec — solving the #1 adoption barrier for cap tracking accuracy.

**6. Miles Earning Insights (F7 — v1.2)**
...
|-------|----------|--------------------------|
| **✅ SHIPPED: S16-17** | **iOS Auto-Capture (F26) + Android Auto-Capture (F27)** | **Solves manual logging friction — highest-impact adoption barrier. iOS Shortcuts API (2-3 sec) + Android NotificationListenerService (0 sec passive). Reduces logging effort by 90-95%.** |
...
| **v1.5 (Months 9–11)** | Card Coverage 20→29 cards (F22) + Rate Change Alerts (F23) | Increases addressable market from ~60% to ~85% of SG miles card holders. 29 cards (10 added, 1 removed: POSB Everyday reclassified as cashback). Rate alerts create a "watches out for me" trust signal — deepest defensibility. |
...
- **Bank API / SGFinDex integration**: Not available to fintechs. Auto-capture achieved via iOS Shortcuts API + Android NotificationListenerService instead — native OS integration workaround delivers 0-3 sec logging.
...
**3. The product is disciplined and execution-focused.**
The core MVP shipped five features: card setup, recommendation engine, cap tracker, transaction logging, and rules database. Auto-capture (F26 iOS + F27 Android) was then shipped to solve the manual logging friction — reducing effort by 90-95%. No AI gimmicks. No affiliate conflict. It does one thing precisely — tells the user which card to tap — with accuracy and minimal friction.
...
| Auto-capture adoption rate | 80%+ | iOS Shortcuts setup + Android notification permission; leading indicator of cap tracking accuracy |
| NPS | 50+ | Proxy for word-of-mouth growth |

### The Risk We Are Watching Most Closely
**Auto-capture setup friction on iOS.** F26 requires users to install an iOS Shortcut and enable automation — a one-time 2-minute setup. If users skip this step, they fall back to manual logging (20 sec vs 2-3 sec). Android F27 is passive (zero setup), so iOS onboarding UX is the key adoption lever. Mitigation: In-app guided setup flow with screenshots; community demo videos.
