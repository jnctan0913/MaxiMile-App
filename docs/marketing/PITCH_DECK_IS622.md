...
|---------|-------------|
| Bank API / SGFinDex integration | Not open to fintechs — auto-capture achieved via iOS Shortcuts API + Android NotificationListenerService instead |
...
- 8-category grid (4×2 layout): Dining, Transport, Online, Groceries, Petrol, Bills, Travel, General
...
**How we solved it**:
- **F26 (iOS Auto-Capture — Shipped)**: Apple Pay transaction trigger launches MaxiMile via iOS Shortcuts with pre-filled amount, merchant, and card — user confirms in 2-3 seconds
- **F27 (Android Auto-Capture — Shipped)**: NotificationListenerService parses bank SMS notifications and auto-logs transactions in 0 seconds (passive)
- **Result**: Logging friction reduced from 20 sec (manual) to 0-3 sec (auto-capture) — 90-95% effort reduction
