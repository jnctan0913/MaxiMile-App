...
**Changelog (v2.11)**: Added F43 (Transaction Entry Correction) — users can now edit or delete a wrongly logged transaction directly from the Transactions tab. Swipe-to-reveal actions (Edit gold, Delete red) on each transaction row. Edit opens a pre-filled bottom sheet reusing the Log form; all fields editable (amount, category, card, date). Delete requires a destructive confirmation alert. Both actions immediately recalculate `spending_state` so cap tracking stays accurate. RICE score 1012 (P1). Added Epic E22 (Transaction Data Integrity) and Sprint 35 (1.5 weeks, 14 SP, 5 stories).
**Changelog (v2.10)**: Added F42 (Merchant Search on Recommend Tab) — replaces the original F9 placeholder with a fully specified feature based on `docs/planning/MERCHANT_SEARCH_PLAN.md` and `docs/planning/PRD_MERCHANT_SEARCH.md`. Users can type a merchant name (e.g., "Grab", "Starbucks") in a search bar on the Recommend home screen and get routed to the correct category recommendation without guessing. Client-side fuzzy search over ~200 curated Singapore merchants, 120ms debounce, bills subcategory routing. Added Epic E21 (Merchant-Driven Discovery) and Sprint 34 (2-week sprint, 20 SP, 6 stories). Updated F9 entry to reference F42. See `docs/planning/MERCHANT_SEARCH_PLAN.md` for full PRD + DRD + architecture.
**Changelog (v2.9)**: Sprint status reconciliation. Marked implemented sprints as COMPLETED: Sprint 21 (F30 Petrol/Bills Resolution + F32 Condition Transparency — conditions_note on recommendation cards, HSBC Revolution MCC 5814 exclusion, insurance warning on Bills screen), Sprint 22 (F31 Min Spend Enforcement — migration 020, spending-settings.tsx), Sprint 23 (F33 Part 1 Card Expansion 22→29 — migration 014, all 7 cards with earn rules), Sprint 24 (F33 Part 2 UOB Lady's Solitaire category selection — migration 021, CategorySelectionSheet.tsx), Sprint 29 (F36 Earn Rate Refresh — migration 20260303000009, 7 corrections applied), Sprint 33 (F41 Navigation Restructure — Cards+Caps merged, Transactions tab promoted). Sprints 30-31 remain PLANNED. Updated sprint plan source reference to PRD v2.9.
**Changelog (v2.8)**: Navigation Restructure — Cards+Caps Merge & Transactions Tab. Added F41 (Navigation Restructure) and Epic E20. Merged "My Cards" and "Cap Status" tabs into a single "My Cards" tab showing cap status view with card-tap-to-detail navigation. Promoted Transaction History from Profile (2 taps deep) to a dedicated "Transactions" tab. Card Detail screen now includes "See Transactions" button for card-specific transaction history. Removes navigation redundancy (two tabs showing card lists) and surfaces high-frequency Transaction History at the top level. 5 files modified: `caps.tsx` (card press target + Add Card button), `cards.tsx` (replaced with transaction history), `_layout.tsx` (tab order/labels/icons), `card/[id].tsx` (See Transactions button), `profile.tsx` (removed Transaction History menu item).
**Changelog (v2.7)**: Added F40 (Auto-Capture Setup Carousel) — UX improvement to onboarding Step 1.5. Replaces the 6-item numbered text instruction list in the shortcut setup screen with a swipeable carousel. Each step gets its own visual card with icon illustration, title, and short description. Addresses risk R-670 (iOS Shortcut setup drop-off due to manual steps) by reducing cognitive load. Single-file change, no new dependencies. See `maximile-app/app/auto-capture-setup.tsx`.
**Changelog (v2.6)**: MileLion March 2026 systematic audit — earn rate data refresh. Cross-referenced all 30 cards against MileLion's authoritative 2026 guide (`docs/technical/AUDIT_MILELION_VS_RANKINGS.md`). Found 3 critical (P0) and 5 major (P1) discrepancies. Added F36 (Earn Rate Data Refresh — 7 immediate corrections: DBS Altitude travel bonus removed, HSBC Revolution cap $1,000→$1,500, DBS Woman's World cap $2,000→$1,000, UOB Preferred Platinum added to transport, UOB Lady's Card added to groceries, HSBC Revolution added to travel, KrisFlyer UOB transport 2.0→2.4 mpd). Added F37 (DBS yuu Card Integration — deferred to Sprint 30+), F38 (SC Smart Card Bonus Tiers — deferred to Sprint 30+), F39 (Points Currency Quality Tiebreaker — P2 backlog). Added E17 (Earn Rate Accuracy — March 2026 Refresh). Sprint 29 planned for F36 implementation.
...
| F9 | **Merchant Search** | ~~Original placeholder~~ — Superseded by **F42** (Merchant Search on Recommend Tab). See F42 under P1.5 for full specification | 2000 | 1 | 70% | 3 | 467 | Superseded by F42 |
...
| F25 | **Automated Rate Change Detection** | Automated pipeline that monitors bank T&C documents and detects rate/benefit changes using AI classification. GitHub Actions runs daily against ~35 sources (30 T&C PDFs via HTTP + 5 bank index pages via Playwright). 3-tier change detection: (1) T&C version+date check (cheapest), (2) SHA-256 content hash comparison, (3) Gemini 2.0 Flash AI classification with confidence scoring. PDF text extracted via `pdf-parse`. Entire infrastructure runs at $0/month on free tiers. See `docs/RATE_DETECTION_ARCHITECTURE.md` for full technical spec | 3500 | 3 | 50% | 6 | 875 | Daily scraper checks ~35 T&C sources (30 PDFs + 5 index pages); version+date gating skips unchanged T&Cs (cheapest gate); content-hash gating eliminates unchanged pages (zero LLM cost on no-change days); Gemini 2.0 Flash classifies detected changes with structured JSON output; confidence >=0.85 auto-approved from Tier 1 sources (bank_tc_page, bank_announcement, bank_tc_pdf); 0.50-0.84 queued for admin review; <0.50 auto-discarded; pipeline health dashboard shows source uptime and detection accuracy; detection latency <48 hours |
...
| F41 | **Navigation Restructure — Cards+Caps Merge & Transactions Tab** | Merge "My Cards" and "Cap Status" tabs into a single "My Cards" tab (cap status view with card-tap-to-detail). Promote Transaction History from Profile settings to a dedicated "Transactions" tab. Add "See Transactions" button on Card Detail screen. Remove Transaction History from Profile menu. Reduces navigation redundancy (two separate tabs both showing card lists) and surfaces high-frequency transaction history at the top level. New tab order: My Cards (merged with caps), Transactions, Recommend, Log, Miles | 5000 | 2 | 95% | 0.5 | 19000 | My Cards tab shows cap status view with card images, progress bars, and "Add Card" button; tapping a card navigates to Card Detail (not card-transactions); Transactions tab shows global transaction history grouped by month with pull-to-refresh; Card Detail screen has "See Transactions" button linking to card-specific transaction history; Profile no longer shows Transaction History menu item; cap badge (red dot) appears on My Cards tab when any cap >= 80%; 5 tabs visible: My Cards, Transactions, Recommend, Log, Miles |
| F43 | **Transaction Entry Correction** | Users who log a transaction with the wrong card, category, amount, or date currently have no recourse — the error is permanent and silently corrupts cap tracking and recommendations for the rest of the month. F43 adds swipe-to-reveal Edit and Delete actions on every transaction row in the Transactions tab. **Edit**: Opens a pre-filled bottom sheet reusing the existing Log form components; all fields are editable (amount, category, card, date); saving triggers a Supabase UPDATE and immediate `spending_state` recalculation. **Delete**: Destructive confirmation alert ("Delete transaction? This will adjust your cap tracking.") followed by Supabase DELETE and `spending_state` recalculation. Optional 5-second undo snackbar after delete to reduce accidental-deletion anxiety. UI pattern: swipe-left on row reveals gold Edit button (pencil icon) and red Delete button (trash icon) — consistent with iOS/Android system conventions. No new navigation screens required. | 750 | 3 | 90% | 2 | 1012 | Swipe-left on any transaction row reveals Edit and Delete action buttons; Edit opens pre-filled bottom sheet with all four fields editable; saving UPDATE applies to Supabase and immediately recalculates `spending_state` for affected card/category/month; Delete shows confirmation alert before removing record; after DELETE, `spending_state` spending total decremented accordingly; cap status dashboard and recommendation engine reflect corrected data within <2 seconds; 5-second undo snackbar appears post-delete; analytics events fire: `transaction_edited`, `transaction_deleted` |

#### P1.5 — Onboarding UX Improvements

| # | Feature | Description | Reach | Impact | Confidence | Effort (wks) | RICE Score | Acceptance Criteria |
|---|---------|-------------|-------|--------|------------|--------------|------------|---------------------|
| F40 | **Auto-Capture Setup Carousel** | Replace the 6-item numbered text instruction list on the auto-capture shortcut setup screen (`auto-capture-setup.tsx`) with a swipeable carousel. Each of the 6 setup steps gets its own visual card with an Ionicons icon illustration, step badge, title, and short description. Addresses the "iOS Shortcut setup drop-off due to manual Add Automation step" risk — users find the wall of text confusing and are less likely to complete setup. Carousel reduces cognitive load by presenting one step at a time with visual context. Uses `FlatList` (horizontal + `pagingEnabled`) with pagination dots — no new dependencies. Single-file change. Everything else on the setup screen stays the same (title, subtitle, download button, iOS 17+ badge, "I've set this up" button) | 3500 | 1 | 90% | 0.5 | 6300 | 6-slide horizontal carousel replaces the numbered text instruction list in the glass card; each slide shows step badge, icon illustration (72px gold circle with Ionicons), title, and description; pagination dots (active = gold pill, inactive = circle) below carousel; carousel swipes left/right on both iOS and web; download button and "I've set this up" button still function correctly; no new npm dependencies added |
| F44 | **Recommend Tab Coach Mark Tour** | First-session walkthrough that introduces the three transaction entry paths on the Recommend tab to new users via a 3-step sequential coach mark overlay. Shown exactly once, triggered on first Recommend tab focus after onboarding completes. Step 1 highlights the Merchant Search bar ("Know where you're spending? Type a merchant name"), Step 2 highlights the Category tile grid ("Spending by category? Tap what you're buying"), Step 3 highlights the FAB button ("At checkout? Tap [FAB name] — we detect where you are and open your best card"). Each step: dimmed overlay + gold spotlight ring cutout over the target element + tooltip bubble with title, description, step dots, Next/Got it, and Skip link. Spotlight uses 4 dim rect Views (no SVG/canvas) inside a transparent Modal (required to cover the absolute-positioned tab bar). Element positions measured via `ref.current.measureInWindow()` with 100ms settle delay. AsyncStorage key `@maximile_recommend_coach_mark_done` prevents repeat display. Also renames the FAB button from "Smart Pay" to a more discoverable name (TBD, see naming decision). Analytics: `coach_mark_shown`, `coach_mark_advanced`, `coach_mark_skipped`, `coach_mark_completed`. | 5000 | 2 | 90% | 1 | 9000 | Coach mark overlay appears on first Recommend tab visit after onboarding; 3 sequential steps highlight Merchant Search, Category Grid, and FAB respectively; each step shows a gold spotlight ring around the target element with a dimmed background; tooltip bubble shows step counter dots, title, description, and Next/Got it button; Skip link visible throughout; tapping Next advances to next step with scroll + re-measure; tapping Got it on step 3 dismisses and writes AsyncStorage key; subsequent Recommend tab visits show no overlay; FAB label updated to new name; analytics events fire correctly for each transition |
| F42 | **Merchant Search on Recommend Tab** | Search bar on the Recommend home screen that lets users type a merchant name (e.g., "Grab", "Starbucks", "NTUC") and get routed to the correct category recommendation. Solves the "Category-Confused Caleb" problem: users think in merchants, not categories, and currently must guess which of 8 tiles maps to their merchant. Client-side fuzzy search over ~200 curated Singapore merchants with prefix/substring ranking, 120ms debounce, no external search libraries. Bills merchants route through subcategory (e.g., Singtel → bills/telco). Recommendation page shows "Best card for [Merchant]" when accessed via search. **Merchant logos**: 162 real brand logos (64×64 PNG) sourced via Google Favicon API + manual curation, displayed in autocomplete dropdown alongside merchant name and category badge. Supersedes original F9 placeholder. See `docs/planning/MERCHANT_SEARCH_PLAN.md` for full PRD + DRD + architecture | 4000 | 2 | 80% | 2 | 3200 | Search bar visible above category tiles on Recommend home screen; autocomplete dropdown shows up to 6 results after ≥ 2 chars (120ms debounce); each result shows merchant name + category badge; selecting a merchant navigates to `/recommend/[categoryId]?merchantName=[name]`; Bills merchants route through subcategory; "No merchants found" empty state with category browse suggestion; recommendation page header shows "Best card for [Merchant]" when `merchantName` param present; search works fully offline (static catalogue); P95 local search latency < 200ms; analytics events: `search_initiated`, `merchant_selected`, `search_abandoned` |
...
| F36 | **Earn Rate Data Refresh (MileLion March 2026 Audit)** | Systematic audit against MileLion's authoritative 2026 guide revealed 7 earn rate/cap discrepancies that can be corrected immediately via SQL: (1) DBS Altitude travel bonus removed — downgrade from 4.0 to 1.2 mpd, (2) HSBC Revolution cap boosted — $1,000→$1,500/mo across all bonus categories, (3) DBS Woman's World cap cut — $2,000→$1,000/mo for online, (4) UOB Preferred Platinum added to transport at 4 mpd (SimplyGo contactless, already in DB but missing from rankings), (5) UOB Lady's Card added to groceries at 4 mpd (supermarkets), (6) HSBC Revolution added to travel at 4 mpd (direct airline/hotel bookings, not OTAs), (7) KrisFlyer UOB transport rate updated from 2.0→2.4 mpd (with $1,000/year SIA spend condition). See `docs/technical/AUDIT_MILELION_VS_RANKINGS.md` for full findings | 5000 | 3 | 95% | 1 | 14250 | DBS Altitude shows 1.2 mpd (not 4.0) for travel; HSBC Revolution cap references updated to $1,500/mo; DBS Woman's World cap updated to $1,000/mo; UOB Preferred Platinum appears in transport rankings at 4 mpd; UOB Lady's Card appears in groceries rankings at 4 mpd; HSBC Revolution appears in travel rankings at 4 mpd (direct bookings only); KrisFlyer UOB shows 2.4 mpd for transport; all existing tests pass; RECOMMENDATION_RANKINGS.md updated |
| F37 | **DBS yuu Card Integration** | Add DBS yuu AMEX and DBS yuu Visa as cards #31-32. These cards earn 10 mpd at yuu merchants (FairPrice, 7-Eleven, Giant) for groceries and 10 mpd for food delivery (Deliveroo, GrabFood, Foodpanda). Would rank #1 in Groceries and #1 in Dining (food delivery) — single largest gap in recommendation engine. **DEFERRED to Sprint 30+** due to complexity (new cards + merchant-specific bonus logic) | 3000 | 3 | 70% | 4 | 1575 | DBS yuu AMEX and DBS yuu Visa added as cards #31-32; 10 mpd grocery bonus rule at yuu merchants with $800/mo cap and $800/mo min spend; 10 mpd food delivery bonus rule (Deliveroo, GrabFood, Foodpanda); both cards appear in Groceries rankings at #1; AMEX acceptance warning shown where relevant |
| F38 | **SC Smart Card Bonus Tiers** | Model SC Smart Card's tiered cashback-to-miles equivalence. Currently listed at 0.4 mpd (base only), but MileLion values it at 5.6 mpd equivalent on transport, fast food, streaming, and EV charging (rising to 7.42 mpd at $800-$1,500/mo total spend, 9.28 mpd at $1,500+). Requires policy decision on showing cashback cards in miles ranking. **DEFERRED to Sprint 30+** pending cashback-to-miles conversion policy | 2000 | 2 | 50% | 3 | 667 | SC Smart Card shows tiered bonus rates (5.6/7.42/9.28 mpd equiv.) for transport, fast food, streaming, EV charging; cashback-to-miles conversion formula documented and applied; tiered spend thresholds modeled in earn_rules conditions; card appears competitively in transport rankings |
| F39 | **Points Currency Quality Tiebreaker** | Add `currency_quality` field (1=best, 3=worst) to cards table. Use as tiebreaker in scoring formula when earn_rate_mpd is equal. MileLion explicitly argues not all 4 mpd is equal — AMEX MR/Citi ThankYou/HSBC Rewards (Tier 1) have more transfer partners and better expiry than DBS Points/OCBC$/Maybank TreatsPoints (Tier 3). **P2 backlog** — current alphabetical tiebreaker is acceptable for v1 | 1500 | 1 | 60% | 2 | 450 | `currency_quality` column added to cards table (1-3); scoring formula uses currency_quality as secondary sort when earn_rate_mpd is tied; cards with better currency rank higher within same mpd tier; UI shows currency tier indicator on recommendation cards |
...
| F9: Merchant Search | **Performance** | Superseded by F42 |
| F42: Merchant Search on Recommend Tab | **Performance** (recommendation access) | Users think in merchants, not categories; search bar eliminates the mental translation step; more merchants matched = more successful recommendations; directly increases MARU by recovering users who bounce off the category grid |
...
| F40: Auto-Capture Setup Carousel | **Performance** (onboarding completion) | Replaces confusing text wall with visual step-by-step carousel; directly improves shortcut setup completion rate; addresses the "Add Automation" drop-off risk with reduced cognitive load per step |
| F44: Recommend Tab Coach Mark Tour | **Performance** (activation) | New users land on the Recommend tab with no mental model of the three entry paths — research shows users familiar with all three paths have significantly higher recommendation completion rates. The coach mark removes first-session confusion at zero ongoing cost (shown once, dismissed forever). Directly addresses activation drop-off: users who don't attempt a recommendation on session 1 rarely return |
...
- As a user viewing the shortcut setup instructions, I want each step presented as a visual card in a swipeable carousel (rather than a wall of numbered text), so that I can focus on one step at a time and feel less overwhelmed by the setup process (F40)
...
- ~~As a user, I want to set my estimated monthly spending level (in settings or onboarding), so that the app knows whether I meet card-specific minimum spend thresholds~~ **[DEFERRED — not included in demo. UI page exists but hidden from navigation; `recommend()` hardcodes spend to 0. Code preserved for post-demo enablement.]**
...
### Epic 21: Merchant-Driven Discovery (F42)
- As a user at a specific merchant (e.g., Shake Shack), I want to type the merchant name in a search bar on the Recommend home screen, so that I get the right card recommendation without guessing which category it belongs to (F42-S1)
- As a user typing a merchant name, I want to see matching suggestions appear as I type with category badges, so that I can quickly select without finishing the full spelling (F42-S2)
- As a user who selects a merchant from search, I want to be routed to the correct category recommendation with "Best card for [Merchant]" in the header, so that the recommendation feels personalized and I know it's for the right merchant (F42-S3/S5)
- As a user whose merchant isn't recognized, I want a helpful "No merchants found" message with a suggestion to browse categories, so that I still get a recommendation rather than a dead end (F42-S4)
- As the product team, I want analytics events for search interactions (initiated, selected, abandoned), so that we can measure adoption and improve the merchant dictionary (F42-S6)

### Epic 22: Transaction Data Integrity (F43)
- As a user who logged the wrong card for a transaction, I want to swipe left on the transaction row to reveal an Edit option, so that I can fix the error before it affects my cap tracking for the rest of the month (F43-S1/S2)
- As a user correcting a logged entry, I want a pre-filled edit form to open as a bottom sheet, so that I only need to change the wrong field without re-entering all data (F43-S2)
- As a user who submitted an edit, I want the cap tracker to immediately reflect the corrected data, so that my recommendations are based on accurate spending totals (F43-S3)
- As a user who logged a duplicate or entirely wrong transaction, I want to delete it after confirming, so that phantom entries don't inflate my cap usage (F43-S4)
- As a user who accidentally deleted a transaction, I want a 5-second undo option, so that a misswipe doesn't permanently corrupt my records (F43-S4)
- As the product team, I want analytics events for edit and delete actions, so that we can identify which fields are most commonly corrected and improve the Log flow accordingly (F43-S5)

### Epic 23: First-Session Activation (F44)
- As a new user landing on the Recommend tab for the first time, I want a brief walkthrough of the three ways to get a recommendation, so that I don't miss the Merchant Search or FAB features and leave without getting value (F44-S1)
...
| **iOS Shortcut setup drop-off due to manual "Add Automation" step** | Medium | Medium | Apple does not allow programmatic Shortcut installation — user must manually tap "Add Automation" in the Shortcuts app. Mitigation: pre-configured `.shortcut` file minimizes effort to ~30 seconds; setup wizard provides step-by-step guidance; onboarding Step 1.5 catches users at peak motivation; **F40 carousel replaces wall-of-text instructions with swipeable visual cards to reduce cognitive load and improve setup completion rate** | Product/UX |
...
```
GitHub Actions (weekly cron) → HTTP fetches 25 MileLion review pages
    → Extract dateModified from JSON-LD structured data
    → Date gate: skip if dateModified matches stored tc_last_updated
    → Date newer → extract article content + fetch our DB data
    → Gemini Flash compares MileLion content vs our DB (Groq Llama fallback)
...
Key design decisions:
- **MileLion pivot (Sprint 16c)**: Switched from 35 bank T&C PDF sources (fragile — 404s, IP blocking, encoding issues) to 25 MileLion review page sources. MileLion maintains authoritative, human-verified credit card reviews with `dateModified` metadata
- **Date-gated detection**: JSON-LD `dateModified` provides a cheap, reliable change signal — no content hashing needed. If the date hasn't changed, the content hasn't changed
- **DB comparison (not content diff)**: AI compares MileLion's article content against our database summary, rather than diffing old vs new page content. This catches both MileLion updates and discrepancies in our own data
- Bank T&C sources paused as backup (not deleted — can reactivate if MileLion becomes unavailable)
- Server-rendered WordPress = simple HTTP fetch, no Playwright/PDF parsing needed
...
| P1 | ~~F9~~ → **F42: Merchant Search on Recommend Tab** | Feature | ✅ Shipped Sprint 34 | Sprint 34 (2 weeks, 20 SP) |
