...
**Feasibility Reference**: `docs/NOTIFICATION_CAPTURE_FEASIBILITY.md`
...
Inherited from existing app design language, plus auto-capture-specific principles:

1. **Auto-magic first** -- Pre-fill everything possible; the user's job is to confirm, not to enter.
2. **Transparent automation** -- Always show the user what was captured and where it came from. No black boxes.
3. **Graceful degradation** -- If auto-capture fails or captures partially, fall back to an editable manual form seamlessly.
4. **Trust through control** -- Users can always edit, dismiss, or disable. Auto-capture never forces a save.
5. **Honest coverage** -- Never imply all transactions are captured. Clearly communicate what works and what doesn't.
6. **Familiar patterns** -- Reuse the existing glassmorphic card style, brand gold (#C5A55A), and bottom sheet patterns.
7. **Platform-native setup** -- iOS setup uses Shortcuts (feels Apple-native). Android setup uses system permission flows (feels Android-native).
...
Consistent with existing MaxiMile design system:

```
/* Auto-Capture Specific */
--source-badge-bg:       #E8F0FE       /* Light blue pill background */
--source-badge-text:     #1A73E8       /* Blue text for source badges */
--success-green:         #34A853       /* Active status, checkmarks */
--warning-amber:         #FBBC04       /* Low-confidence card match */
--confirm-btn-bg:        #C5A55A       /* Brand gold — primary CTA */
--confirm-btn-text:      #FFFFFF       /* White on gold */
--dismiss-btn-border:    #9E9E9E       /* Grey outline for ghost button */
--dismiss-btn-text:      #616161       /* Dark grey text */

/* Existing (reused) */
--brand-gold:            #C5A55A
--card-bg:               rgba(255,255,255,0.08)  /* Glassmorphic card */
...
- [x] WCAG 2.1 AA compliance for all new screens
- [x] VoiceOver (iOS) / TalkBack (Android) full support on confirmation screen
- [x] Source badge announced: "Transaction captured via Apple Pay" / "via bank notification"
- [x] All editable fields have clear accessible labels and edit hints
- [x] Minimum 44pt touch targets for Confirm, Dismiss, and edit icons
- [x] Color not used as sole indicator — icons + text accompany all status colors
- [x] Privacy disclosure screen fully readable by screen reader
- [x] Setup wizard navigable via accessibility gestures (swipe between steps)
